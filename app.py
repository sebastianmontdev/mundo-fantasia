...
from funciones import mostrar,agregar,calcular,buscar,eliminar,actualizar,agregar1
...
ventas = [
    ...
    {"cliente" : "jose","producto" : "habitos atomicos","cantidad": 50,"fecha": "sabado","precio": 20000},
    ...
    if opcion == 1:
        ...
        while True:
            if chose == "yes":
                ...
                print(ventas)
                producto_vendido = nuevo_usuario["title"]
                cantidad_vendida = nuevo_usuario["amount"]
                mostrar(inventario)
                print(f"\n⏳ Buscando '{producto_vendido}' en el inventario para actualizar...")

                inventario_actualizado = False
                for item in inventario:
                    # Comprobar si el 'title' del inventario coincide con el 'producto' vendido
                    if item["title"] == producto_vendido:
                        if item["amount"] >= cantidad_vendida:
                            # Reducir la cantidad
                            item["amount"] -= cantidad_vendida
                            inventario_actualizado = True
                            print(f"✅ ¡Inventario actualizado! La nueva cantidad de '{producto_vendido}' es: {item['amount']}")
                            mostrar(inventario)
                        else:
                            print(f"⚠️ Stock insuficiente. Solo quedan {item['amount']} unidades de '{producto_vendido}'.")
                        break # Salir del bucle una vez que el producto ha sido encontrado/procesado

                if not inventario_actualizado:
                    print(f"❌ Error: El producto '{producto_vendido}' no se encontró en el inventario.")
                #i ask to user if he want to add another book,if the anwser is yes we start a new register,if not we go out the loop 
